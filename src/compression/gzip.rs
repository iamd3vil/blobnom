...
impl GzipCompressor {
    pub fn new(level: Option<u32>) -> Self {
        Self {
            level: level.unwrap_or(6), // Default compression level
        }
    }
...
impl Compressor for GzipCompressor {
    async fn compress(&self, data: &[u8]) -> io::Result<Vec<u8>> {
        ...
        let mut encoder = GzipEncoder::with_quality(buf_reader, level);
...
mod tests {
    ...
    fn test_new_with_custom_level() {
        let compressor = GzipCompressor::new(Some(3));
        assert_eq!(compressor.level, 3);
    }

    #[tokio::test]
    async fn test_compress_decompress_roundtrip() {
        let compressor = GzipCompressor::new(Some(9));
    ...
    async fn test_compress_empty_data() {
        let compressor = GzipCompressor::new(None);
        let data = b"";
